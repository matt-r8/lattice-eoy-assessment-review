#!/usr/bin/env python3
"""
fetch_missing_software.py - Pull 5 missing Software practice EOY assessments

Fetches only these specific members:
1. Angie Davidson
2. Nate Enders
3. Gannon Gardner
4. Kevin Nguyen
    ...
    for attempt in range(retries):
        try:
            ...
            if e.code == 429:  # Rate limit
                print(f"   ⚠️  Rate limit hit (HTTP 429), retrying in {retry_delay}s...")
...
def process_single_person(name, cycle, reviewees, output_dir, cycle_name):
    ...
    try:
        ...
        # Rate limit protection: 2-second delay between requests
        time.sleep(2)
...
def main():
    """Fetch 5 missing Software practice assessments and save as markdown."""

    # Configuration
    CYCLE_NAME = "2025 EOY Assessment"
    BASE_OUTPUT_DIR = Path(__file__).parent / "assessments" / "Software"
    BASE_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    # 5 missing Software practice members
    MISSING_NAMES = [
        "Angie Davidson",
        "Nate Enders",
        "Gannon Gardner",
    ...
    print(f"\n{'='*60}")
    print(f"📁 Processing 5 missing Software practice members")
    print(f"{'='*60}")
    ...
    for i, name in enumerate(MISSING_NAMES, 1):
        print(f"\n[{i}/{len(MISSING_NAMES)}]", end=" ")
        success = process_single_person(name, cycle, reviewees, BASE_OUTPUT_DIR, CYCLE_NAME)
        if success:
            success_count += 1
        else:
            failed_count += 1
    ...
    print(f"   - Successfully pulled: {success_count}/{len(MISSING_NAMES)} assessments")
    if failed_count > 0:
